...
from arcan.forge.service.token import TokenService
...
async def register_user(user_create: UserCreate, db: AsyncSession = Depends(session_scope)):
    result = await UserService(user_repository=UserRepository(db), pwd_context=pwd_context).register_user(user_create)
    return result
...
#     # store on Token db table
#     token = Token(access_token=access_token, token_type="bearer")
    
#     await TokenService(token_repository=TokenRepository(db)).register_token(token)
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), db: AsyncSession = Depends(session_scope)):
    ...
    token_service = TokenService(token_repository=TokenRepository(db))
    await token_service.register_token(token)
