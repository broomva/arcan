...
fn map_status(s: &str) -> SandboxStatus {
    match s {
        ...
        "error" => SandboxStatus::Failed { reason: "provider reported error".into() },
...
fn map_status_error(
    ...
    match status.as_u16() {
        404 => SandboxError::NotFound(
            sandbox_id.cloned().unwrap_or_else(|| SandboxId("unknown".into())),
        ),
        402 | 403 => SandboxError::CapabilityDenied { capability: "api_access" },
        408 | 504 => SandboxError::ExecTimeout {
            sandbox_id: sandbox_id.cloned().unwrap_or_else(|| SandboxId("unknown".into())),
            timeout_secs: 0,
        },
...
impl SandboxProvider for VercelSandboxProvider {
    ...
    async fn create(&self, spec: SandboxSpec) -> Result<SandboxHandle, SandboxError> {
        ...
        let persistent =
            !matches!(spec.persistence, arcan_sandbox::types::PersistencePolicy::Ephemeral);
    ...
    async fn list(&self) -> Result<Vec<SandboxInfo>, SandboxError> {
        ...
        list_resp.sandboxes.into_iter().map(vercel_sandbox_to_info).collect()
    }
}
...
mod tests {
    ...
    fn provider_for(server: &mockito::Server) -> VercelSandboxProvider {
        VercelSandboxProvider::new("test-token", None)
    ...
    fn running_sandbox_json(id: &str, name: &str) -> String {
        format!(
            r#"{{"id":"{id}","name":"{name}","status":"running","createdAt":"{CREATED_AT}"}}"#
        )
    }
    ...
    fn capabilities_include_network_outbound() {
        let p = VercelSandboxProvider::new("tok", None);
        assert!(p.capabilities().contains(SandboxCapabilitySet::NETWORK_OUTBOUND));
    }
