...
- arcand **aborts** mid-stream on a tool call (e.g. `knowledge_search`), then `create_session` returns 502 until the container restarts (BRO-1483) → a synchronous `Tool::execute` that nests `tokio::runtime::Handle::block_on` (cross-session/journal tools do this) was called *directly on a Tokio worker thread* by `ArcanHarnessAdapter::execute`; `block_on` from within an async context panics ("Cannot block the current thread from within a runtime"), and the release profile's `panic = "abort"` turns that panic into a whole-process abort → run the sync tool on the blocking pool: `ArcanHarnessAdapter::execute` now wraps `tool.execute()` in `tokio::task::spawn_blocking(...)`, where nested `block_on` and blocking I/O are legal, so a tool fault stays a structured `ToolCallFailed` instead of a crash. **Rule**: any `Tool::execute` that blocks or nests a runtime must be driven from a blocking thread, never an async worker — the harness now guarantees this for every registry tool.
